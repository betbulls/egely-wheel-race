...
// Badge art: the custom SVG family (assets/achievement-icons/<id>.svg — the
// "aura disc" chassis, generated by _generate.js there) with a bullet-proof
// fallback: if a file is missing (partial deploy, new badge without art yet)
// the original emoji takes its place, so no surface ever breaks.
export function iconHtml(a){
  const emoji = String(a.icon || '🏅').replace(/['"<>&]/g, '');
  return `<img class="ai-img" src="assets/achievement-icons/${a.id}.svg" alt="" loading="lazy" `
    + `onerror="var s=document.createElement('span');s.className='ai-emoji';s.textContent='${emoji}';this.replaceWith(s)">`;
}
