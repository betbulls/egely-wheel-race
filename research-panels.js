...
function buildPipeline(samples, coef, floorTau, coasts){
  ...
  // EVERY free decay arms its own ghost — not just the last one in the range.
  // (Owner feedback on a real run: re-spin the wheel three times inside one
  // coast window and the strip stayed blank until the final decay. A decay
  // segment ends where the wheel is re-driven; driving itself draws nothing.)
  const armScan = (iFrom, iTo) => {
    const out = [];
    let armed = null;
    for(let i = Math.max(2, iFrom); i <= iTo; i++){
      if(armed && samples[i].rpm > samples[i - 1].rpm * 1.2 && samples[i].rpm > 3){
        out.push({ ...armed, tEnd: samples[i - 1].t });   // re-driven: close this decay here
        armed = null;
      }
    ...
    if(armed) out.push(armed);   // survives to the end of the range
    return out;
  };
  const integrateGhost = (armed, tMax) => {
    ...
    // the 0-point only on a NATURAL stop — a segment clipped by a re-spin
    // must not fabricate a predicted standstill at the clip time
    const stopped = w <= 0.05;
    if(stopped) pts.push({ t, rpm: 0 });
    return { t0: armed.t0, w0: armed.w0, pts, tStop: stopped ? t : null };
  };
  const ghosts = [];
  let lastCoastEnd = -1;
  if(coasts && coasts.length){
    for(const cs of coasts){
      ...
      for(const armed of armScan(iFrom, iTo))
        ghosts.push(integrateGhost(armed, Math.min(armed.tEnd != null ? armed.tEnd : Infinity, to + 5)));
      lastCoastEnd = Math.max(lastCoastEnd, to);
    }
  }
  // armings AFTER the last closed coast (the in-progress decays)
  for(const armed of armScan(2, n - 1)){
    if(armed.t0 <= lastCoastEnd) continue;
    ghosts.push(integrateGhost(armed, armed.tEnd != null ? armed.tEnd : armed.t0 + 700));
  }
...
export function createPanelStack(host, opts){
  ...
  const PANELS = {
    timeline: {
      ...
      formula: '<span class="fn">Speed timeline + ghost</span><i>ω</i> = 6000 / counter <span class="fu">rpm</span><span class="fsep">·</span>ghost: d<i>ω̂</i>/d<i>t</i> = −(<i>A</i> + <i>B</i>·<i>ω̂</i> + <i>K</i>·<i>ω̂</i><sup>1.5</sup>)',
      explain: '<p><b>The one question this chart answers: what did the wheel actually do?</b></p><ul><li>The dark line is the measured speed — the raw truth of the run.</li><li>The dashed grey <b>ghost</b> starts wherever the wheel is let go and shows what it would do left completely alone — that prediction comes from your calibration.</li><li>Real line ABOVE the ghost = the wheel kept more speed than it could on its own → something fed it. Below = something braked it extra.</li><li>Violet triangles are your markers; pale red stripes are lost radio — no data there, not zero.</li></ul><p><b>In practice:</b> glance here first — every other panel is a magnifying glass on some part of this picture. The 24 rpm rule marks where the device’s own dial tops out; above it the app reads the internal counter.</p>',
      draw(ctx, W, H){
        if(!samples.length){
        ...
        if(p.ghost && p.ghost.tStop != null && p.ghost.tStop >= view.t0 && p.ghost.tStop <= view.t1){
    ...
    torque: {
      ...
      formula: '<span class="fn">Drive-torque instrument</span><i>τ</i><sub>drive</sub> = <i>I</i>·(<i>α</i><sub>meas</sub> − <i>α</i><sub>base</sub>(<i>ω</i>))<span class="fsep">·</span><i>I</i> = 1.7×10<sup>−7</sup> <span class="fu">kg·m²</span>',
      explain: '<p><b>The one question: right now, is anything pushing or braking the wheel beyond its normal friction and air?</b></p><ul><li>Your calibration says how the wheel slows on its own. Whatever the measurement shows beyond that becomes a <b>push</b> (violet, right) or a <b>drag</b> (amber, left) on the meter.</li><li>The grey band is the calibration’s own wobble. A reading INSIDE it means nothing — the wheel varies this much by itself.</li><li>Only readings that leave the band count. The strip below is the same needle drawn over time.</li><li>Blank stretches = no clean data to estimate from (radio gap or standstill) — blank, never zero.</li></ul><p><b>In practice:</b> a hand-spin is itself a huge push and pins the meter — the interesting part is what the needle does when nothing should be touching the wheel. If it sits outside the band then, something real is acting.</p>',
      draw(ctx, W, H){
        if(!samples.length){
          ctx.fillStyle = FAINT; ctx.font = '11.5px Inter, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ...
    impulse: {
      ...
      formula: '<span class="fn">Drive impulse</span><i>J</i>(<i>t</i>) = <span class="fint">∫</span> <i>τ</i><sub>drive</sub> d<i>t</i> <span class="fu">nN·m·s</span><span class="fsep">·</span><i>J</i><sub>out</sub> = <span class="fint">∫</span> sign(<i>τ</i>)·max(|<i>τ</i>|−<i>B</i>(<i>ω</i>), 0) d<i>t</i>',
      explain: '<p><b>The one question: has a small push been quietly adding up?</b></p><ul><li>The line climbs while something pushes, sinks while something drags, and holds level when nothing happens.</li><li>A push far too small to see on the meter above still shows here — IF it keeps coming back. Adding up is the whole point of this chart.</li><li>The grey band widens with time: that is how far the total could drift on the calibration’s wobble alone. A line wandering inside it = nothing beyond ordinary physics.</li></ul><p><b>In practice:</b> your spin-ups dominate this total (each one is a big push), so judge the hands-off stretches. The note on the chart counts exactly how long the reading spent outside the band.</p>',
      draw(ctx, W, H){
        if(!samples.length){
          ctx.fillStyle = FAINT; ctx.font = '11.5px Inter, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ...
    logspeed: {
      ...
      formula: '<span class="fn">Log-scale speed</span><i>y</i> = log <i>ω</i><span class="fsep">·</span><i>ω</i> = 6000 / counter <span class="fu">rpm</span>',
      explain: '<p><b>The one question: what did the slow end really look like?</b></p><ul><li>The same measured speed as the main chart, redrawn so 2 rpm gets as much room as 200 — each step down the scale roughly halves the speed.</li><li>Small squares parked on the floor = below 1.6 rpm, near standstill.</li><li>The dashed lines are the same calibration ghosts as on the main chart.</li></ul><p><b>In practice:</b> the crawl — roughly 1 to 5 rpm — is the wheel’s most sensitive range: the smallest influence moves it there first. On the normal chart that range is a flat smudge; here a restart from stillness, or a die-down that drags on suspiciously long, is plainly visible.</p>',
      draw(ctx, W, H){
        const { xOf } = timeAxis(W);
        const yOf = logY(H);
        drawTimeGrid(ctx, W, H, xOf); drawGaps(ctx, W, H, xOf);
    ...
    deviation: {
      ...
      formula: '<span class="fn">Ghost deviation strip</span><i>Δ</i> = 100 · (<i>ω</i> / <i>ω̂</i> − 1) <span class="fu">%</span>',
      explain: '<p><b>The one question: during a free slow-down, was the wheel ahead of or behind its prediction — and by how many percent?</b></p><ul><li>The middle line is 0% = exactly the slow-down your calibration predicts. Above it the wheel turns faster than predicted → something feeds it; below, slower → something brakes it.</li><li>The grey band is how closely your calibration repeats itself (the ±% written on the strip). Inside it the difference means nothing — the wheel wobbles that much on its own.</li><li>The line exists only during free decays: while a hand drives the wheel there is no prediction to compare with, so the strip stays honestly blank — and every decay gets its own fresh comparison.</li></ul><p><b>In practice:</b> this is a magnifying glass — a few percent you would never spot on the speed chart is a clear step here. A steady +5% through a whole decay, outside the band, is worth attention; a jittery wiggle inside the band is just the wheel being a wheel.</p>',
      draw(ctx, W, H){
        const p = ensurePipe();
        const { xOf } = timeAxis(W);
        drawTimeGrid(ctx, W, H, xOf);
    ...
    phase: {
      ...
      explain: '<p><b>The one question: does the wheel brake the way it always does?</b></p><ul><li>The dashed curve is your calibration: how hard the wheel brakes at each speed on its own. This chart files every moment by SPEED, not by time.</li><li>Dot ON the curve = boring — normal coasting.</li><li>Dot BELOW the curve (violet) = less braking than normal → something is feeding the wheel energy.</li><li>Dot on the zero line = the speed is being HELD — impossible on its own; something supplies exactly what friction eats.</li><li>Dot below zero = the wheel is actually speeding up. Dot above the curve (amber) = extra braking.</li></ul><p><b>A concrete example:</b> hold the wheel at a steady 14 rpm and a violet column of dots grows at 14, down on the zero line — about 1.5 rpm/s of braking has visibly “vanished”.</p><p><b>In practice:</b> because time does not matter here, a scattered, repeated influence at one speed pools into one visible column. The tinted zone below 5 rpm never gets a verdict (readings are too mushy there and drafts dominate), and past 24 rpm the curve is an extension of the formula, not a measurement.</p>',
      draw(ctx, W, H){
        const p = ensurePipe();
    ...
    byspeed: {
      ...
      formula: '<span class="fn">Drive by speed band</span>mean <i>τ</i><sub>drive</sub> per <i>ω</i>-band ± sd<span class="fsep">·</span>faded = outside the calibration\'s fitted range',
      explain: '<p><b>The one question: at WHICH speeds did the influence appear?</b></p><ul><li>Each bar is the average push (violet, up) or drag (amber, down) while the wheel was in that speed band — over the whole recording.</li><li>The grey block behind each bar is the calibration’s wobble for that band. A bar inside its block means nothing.</li><li>The faded bands above 24 rpm: the calibration never measured up there, so the comparison is an extension — treat it with suspicion.</li><li>“n=” is how many readings a band holds; a band with too few prints its count instead of pretending a result.</li></ul><p><b>In practice:</b> a genuinely speed-tied effect shows as one or two bands clearing their grey block while the rest stay flat. Same-direction bars across ALL measured bands usually mean a speed-independent influence — or a calibration that no longer matches the room.</p>',
      draw(ctx, W, H){
        const p = ensurePipe();
    ...
    revs: {
      ...
      formula: '<span class="fn">Revolutions integral</span><i>N</i>(<i>t</i>) = <span class="fint">∫</span> <i>ω</i>/60 d<i>t</i> <span class="fu">rev</span>',
      explain: '<p><b>The one question: how much did the wheel actually turn?</b></p><ul><li>The line only ever climbs — steeper = spinning faster, flat = standing still.</li><li>The dashed line carries the count on the way plain coasting would (from the last release, using the calibration). The real line pulling above it = more turns than coasting alone explains.</li><li>Turns during radio gaps are not counted — the line holds level rather than guessing.</li></ul><p><b>In practice:</b> this is the most quotable simple number of a session (“58 turns in three minutes”), and the header’s Turns tile is exactly this line’s final value.</p>',
      draw(ctx, W, H){
        const p = ensurePipe();
        const { xOf } = timeAxis(W);
        drawTimeGrid(ctx, W, H, xOf); drawGaps(ctx, W, H, xOf);
    ...
    energy: {
      ...
      formula: '<span class="fn">Kinetic energy</span><i>E</i> = ½ · <i>I</i> · <i>ω</i><sup>2</sup><span class="fsep">·</span>0.54 µJ at 24 rpm',
      explain: '<p><b>The one question: how much motion-energy is in the wheel right now?</b></p><ul><li>The line follows speed: it climbs when the wheel speeds up, falls as it slows. The unit is millionths of a joule — the wheel is feather-light.</li><li>Every rise means energy went IN from somewhere: hand, air, anything. Every fall means friction and air ate it.</li><li>The faint dashed line at 0.54 µJ marks the energy at 24 rpm — the top of the device’s own dial.</li></ul><p><b>In practice:</b> drag across any stretch to read exactly how much the wheel gained or lost there. This chart does not use the calibration at all — it is pure measured speed and the wheel’s known inertia, so it is the most assumption-free picture in the stack.</p>',
      draw(ctx, W, H){
        const p = ensurePipe();
        const { xOf } = timeAxis(W);
        drawTimeGrid(ctx, W, H, xOf); drawGaps(ctx, W, H, xOf);
    ...
    work: {
      ...
      formula: '<span class="fn">Work ledger</span><i>W</i>(<i>t</i>) = Δ<i>E</i>(<i>t</i>) + <span class="fint">∫</span> <i>τ</i><sub>base</sub>(<i>ω</i>)·<i>ω</i> d<i>t</i> <span class="fu">µJ</span>',
      explain: '<p><b>The one question: over the run, how much energy reached the wheel from outside?</b></p><ul><li>Your calibration says how much energy the wheel burns to friction and air at every speed. This line adds up what its motion gained PLUS what it burned — together that is exactly what came in from outside: hand, air, anything.</li><li>Flat at zero = nothing came in. Steps up = energy arrived — your spin-ups are the big steps (that is your hand’s energy, measured).</li><li>The grey band is how far the total could drift on the calibration’s wobble alone; it grows the longer the wheel keeps turning.</li></ul><p><b>In practice:</b> the header’s “Energy in, total” is this line’s final value. The interesting kind of movement is a slow climb OUTSIDE the band during a hands-off stretch. Remember it is a net figure — a push and an equal extra drag cancel out.</p>',
      draw(ctx, W, H){
        const p = ensurePipe();
        const { xOf } = timeAxis(W);
        drawTimeGrid(ctx, W, H, xOf); drawGaps(ctx, W, H, xOf);
    ...
    coasts: {
      ...
      formula: '<span class="fn">Coast comparator</span><i>R</i> = <i>T</i><sub>meas</sub>(24→5) / <i>T</i><sub>cal</sub>(24→5)<span class="fsep">·</span><i>T</i><sub>cal</sub> = ' + '—',
      explain: '<p><b>The one question: each time the wheel spun down freely, did it take longer or shorter than it should?</b></p><ul><li>Every free slow-down is timed like a stopwatch lap between speed gates (24→10, 24→5, 12→6, 6→3) and compared with the times your calibration predicts. The tick on each bar is the predicted time.</li><li>×1.00 = exactly as calibrated. ×1.15 = it kept spinning 15% longer — something helped it along. ×0.90 = something braked it.</li><li>On the gate row, violet Δ = longer than predicted, amber = shorter. A struck-through card was interrupted before finishing — its times are incomplete.</li></ul><p><b>In practice:</b> these are the most defensible numbers a run produces — you could verify them with a kitchen stopwatch. If you quote one number from a session, quote a gate time.</p>',
      renderDom(el){
    ...
    quality: {
      ...
      formula: '<span class="fn">Sample cadence</span>Δ<i>t</i> = <i>t</i><sub>i</sub> − <i>t</i><sub>i−1</sub><span class="fsep">·</span>expected ≈ 0.7 s at any wheel speed',
      explain: '<p><b>The one question: can I trust the data of this run?</b></p><ul><li>Each dot is the wait since the previous reading — normally they sit on the dashed 0.7 s line, because that is the device’s own sending pace.</li><li>Red dots are radio dropouts. The same stretches show as pale red stripes on the other charts, and everything derived there goes blank rather than guessed.</li><li>Below about 2 rpm readings naturally arrive slower — that is the device, not a fault.</li><li>The bottom line re-runs the push-or-drag estimate with shorter and longer smoothing; if the size changes with the smoothing, treat that size with suspicion — the direction usually survives.</li></ul><p><b>In practice:</b> before quoting anything from a strange-looking run, check here first — most “weird moments” on the charts are just missing radio, and this panel shows exactly where.</p>',
      draw(ctx, W, H){
        const p = ensurePipe();
        const { xOf } = timeAxis(W);
        drawTimeGrid(ctx, W, H, xOf); drawGaps(ctx, W, H, xOf);
  ...
  PANELS.coasts.formula = '<span class="fn">Coast comparator</span><i>R</i> = <i>T</i><sub>meas</sub>(24→5) / <i>T</i><sub>cal</sub>(24→5)<span class="fsep">·</span><i>T</i><sub>cal</sub>(24→5) = ' + (T_CAL.t24_5 ? T_CAL.t24_5.toFixed(1) + ' s' : '—');
    ...
    stackEl.innerHTML = visibleOrder().map(id => {
      ...
      <div class="rsp-card" data-panel="${id}">
        ...
        <div class="rsp-body" data-body="${id}" ${open ? '' : 'hidden'}>
          ...
          <details class="rsp-fdet what"><summary>What you see</summary>
            <div class="rsp-explain">${P.explain}</div>
          </details>
          <details class="rsp-fdet"><summary>Exact definition</summary>
            <div class="rsp-formula">${P.formula}</div>
          </details>
        </div>
      </div>`;
    }).join('');
  ...
  function exportPng(id){
    ...
    // the formula is HTML for on-screen typesetting — flatten it for the stamp
    ctx.fillText(P.formula.replace(/<sup>/g, '^').replace(/<sub>/g, '_')
      .replace(/<span class="fsep">·<\/span>/g, '  ·  ').replace(/<[^>]+>/g, ''), 12, 50);
...
function styles(){
  ...
  .rsp-explain{color:#4d5a66;font-size:12.5px;line-height:1.62;margin:6px 0 0}
  .rsp-explain b{color:#011624;font-weight:700}
  .rsp-explain p{margin:8px 0 0}
  .rsp-explain p:first-child{margin-top:0}
  .rsp-explain ul{margin:8px 0 0;padding-left:20px}
  .rsp-explain li{margin:4px 0}
  .rsp-fdet{margin-top:8px}
  /* the plain-language reading guide gets a touch more presence than the formula */
  .rsp-fdet.what{margin-top:10px}
  .rsp-fdet.what>summary{color:#67737c}
  ...
  /* The exact definition, typeset like maths rather than dumped as monospace:
     serif italics for variables, true sub/superscripts, a tall integral sign. */
  .rsp-formula{font-family:Georgia,'Times New Roman',serif;font-size:14px;color:#27384e;
    background:#fbfbfc;border:1px solid #eef1f3;border-radius:8px;padding:9px 13px;margin-top:6px;
    overflow-x:auto;white-space:nowrap;line-height:1.7}
  .rsp-formula i{font-style:italic}
  .rsp-formula sub,.rsp-formula sup{font-size:68%;line-height:0}
  .rsp-formula .fn{font-family:'Montserrat',sans-serif;font-size:9px;font-weight:700;letter-spacing:.08em;
    text-transform:uppercase;color:#99a2a7;margin-right:12px;vertical-align:1px}
  .rsp-formula .fu{font-family:'Inter',sans-serif;font-size:11px;color:#99a2a7;margin-left:2px}
  .rsp-formula .fsep{color:#c9ced2;margin:0 10px}
  .rsp-formula .fint{font-size:19px;vertical-align:-2px;line-height:0}
